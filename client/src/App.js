  ...
  function onSignup(event) {
    ...
    fetch("/signup", {
      ...
      if (response.ok) {
        ...
        alert(
          "Error: Username already exists or invalid Username! Please try again!"
        );
      } else {
        alert(`Error: ${response.status} ${response.statusText}`);
      }
        ...
        <Switch>
          ...
          <Route path="/todaysfeed">
            {user ? (
              <Feed2
                outfits={outfits}
                onAddHanger={handleAddHanger}
                user={user}
                wgUsers={wgUsers}
                hangersInCloset={hangersInCloset}
              />
            ) : (
              "Please log in to view today's feed"
            )}
          </Route>
          <Route path="/users">
            {user ? (
              <Users wgUsers={wgUsers} outfits={outfits} />
            ) : (
              "Please log in to see the users of Weather Gram"
            )}
          </Route>
          <Route path="/outfitinspo">
            {user ? (
              <OutfitInspo outfits={outfits} user={user} />
            ) : (
              "Please log in to see outfit inspiration"
            )}
          </Route>
          <Route path="/outfitbreakdown">
            {user ? (
              <OutfitBreakdown outfits={outfits} user={user} />
            ) : (
              "Please log in to shop the looks"
            )}
          </Route>
          <Route path="/mycloset">
            {user ? (
              <MyCloset
                ...
                onUpdateOutfitInCloset={handleUpdateOutfitInCloset}
              />
            ) : (
              "loading"
            )}
