...
export default function PopUpClosetNew({
  wgUsers,
  user,
  hangerInCloset,
  onUpdateOutfitInCloset,
  setHangersInCloset,
}) {
  ...
  function handleDelete() {
    ...
    fetch(`http://localhost:3000/hangers/${hangerInCloset.id}`, {
      ...
      setHangersInCloset((hangersInCloset) =>
        hangersInCloset.filter((hanger) => hanger.id !== hangerInCloset.id)
      );
    });
  }
    ...
    <>
      {modal ? (
        <div className="relative">
          ...
          <div>
            <button
              className="absolute visible text-black"
                      ...
                      {hangerInCloset.outfit.top ? (
                        <>
                          <a
                            ...
                            target="_blank"
                            href={hangerInCloset.outfit.top.zalando_link}
                          >
                            <img
                              className="w-full h-full p-1 lg:p-0 mr-2 lg:mr-0 object-cover"
                              src={hangerInCloset.outfit.top.top_img}
                              alt=""
                            />
                          </a>
                          <a
                            className="block md:w-32 h-32 border border-transparent hover:border-gray-400"
                            target="_blank"
                            href={hangerInCloset.outfit.bottom.zalando_link}
                          >
                            <img
                              className="w-full h-full p-1 lg:p-0 mr-2 lg:mr-0 object-cover"
                              src={hangerInCloset.outfit.bottom.bottom_img}
                              alt=""
                            />
                          </a>
                          <a
                            className="block md:w-32 h-32 border border-transparent hover:border-gray-400"
                            target="_blank"
                            href={hangerInCloset.outfit.shoe.zalando_link}
                          >
                            <img
                              className="w-full h-full p-1 lg:p-0 mr-2 lg:mr-0 object-cover"
                              src={hangerInCloset.outfit.shoe.shoe_img}
                              alt=""
                            />
                          </a>
                        </>
                  ...
                  {user.username === hangerInCloset.outfit.created_by ? (
                    <>
                      <div className="flex flex-wrap mb-14 items-center">
                        ...
                        >
                          <a
                            className="block mb-4 mt-2 lg:mb-0 lg:mr-6 bg-orange-300 hover:bg-orange-400 text-center text-white font-bold font-heading py-5 px-8 rounded-md uppercase transition duration-200"
                            href="#"
                          >
                            Delete From Gram
                          </a>
